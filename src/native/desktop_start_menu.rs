use super::app::{ContextMenuAction, RobcoNativeApp};
use super::desktop_app::{DesktopShellAction, DesktopWindow};
use super::desktop_connections_service::connections_macos_disabled;
...
// ── impl RobcoNativeApp — start menu ─────────────────────────────────────
...
impl RobcoNativeApp {
    ...
    pub(super) fn start_leaf_menu_target(
        ...
        match action {
            ...
            NativeStartLeafAction::LaunchGameProgram(name)
                if name != super::donkey_kong::BUILTIN_DONKEY_KONG_GAME =>
            {
    ...
    pub(super) fn start_leaf_items(&self, leaf: StartLeaf) -> Vec<NativeStartLeafEntry> {
        let builtin_text_editor = super::editor_app::EDITOR_APP_TITLE;
        let _builtin_nuke_codes = builtin_text_editor; // reuse constant name pattern
        // Actually re-derive these from the constants in app.rs scope
        let builtin_nuke_codes_app = "Nuke Codes";
        let builtin_text_editor_app = super::editor_app::EDITOR_APP_TITLE;
        match leaf {
            ...
            StartLeaf::Games => start_game_entries(super::donkey_kong::BUILTIN_DONKEY_KONG_GAME),
        }
    }
    ...
    // ── Styling helpers (also used by other modules) ─────────────────────
