...
use crate::config::{get_current_user, load_apps, save_apps, load_games, save_games, load_networks, save_networks};
use crate::launcher::with_suspended;
use crate::ui::{Term, run_menu, input_prompt, confirm, flash_message, box_message, MenuResult};

// ── Package manager detection ─────────────────────────────────────────────────

#[derive(Debug, Clone, Copy)]
enum PackageManager { Brew, Apt, Dnf, Pacman, Zypper }
...
impl PackageManager {
    ...
    fn install_cmd(self, pkg: &str) -> Vec<String> {
        match self {
            ...
            PackageManager::Apt    => vec!["sudo".into(), "apt".into(), "install".into(), "-y".into(), pkg.into()],
            PackageManager::Dnf    => vec!["sudo".into(), "dnf".into(), "install".into(), "-y".into(), pkg.into()],
            PackageManager::Pacman => vec!["sudo".into(), "pacman".into(), "-S".into(), "--noconfirm".into(), pkg.into()],
            PackageManager::Zypper => vec!["sudo".into(), "zypper".into(), "-n".into(), "install".into(), pkg.into()],
        }
    }
    ...
    fn remove_cmd(self, pkg: &str) -> Vec<String> {
        match self {
            ...
            PackageManager::Apt    => vec!["sudo".into(), "apt".into(), "remove".into(), "-y".into(), pkg.into()],
            PackageManager::Dnf    => vec!["sudo".into(), "dnf".into(), "remove".into(), "-y".into(), pkg.into()],
            PackageManager::Pacman => vec!["sudo".into(), "pacman".into(), "-R".into(), "--noconfirm".into(), pkg.into()],
            PackageManager::Zypper => vec!["sudo".into(), "zypper".into(), "-n".into(), "remove".into(), pkg.into()],
        }
    }
    ...
    fn update_cmd(self, pkg: &str) -> Vec<String> {
        match self {
            ...
            PackageManager::Apt    => vec!["sudo".into(), "apt".into(), "upgrade".into(), "-y".into(), pkg.into()],
            PackageManager::Dnf    => vec!["sudo".into(), "dnf".into(), "upgrade".into(), "-y".into(), pkg.into()],
            PackageManager::Pacman => vec!["sudo".into(), "pacman".into(), "-U".into(), pkg.into()],
            PackageManager::Zypper => vec!["sudo".into(), "zypper".into(), "-n".into(), "update".into(), pkg.into()],
        }
    }
        ...
        Command::new(bin).args(args).output()
            ...
            .map(|o| {
                String::from_utf8_lossy(&o.stdout)
                    .lines()
                    .filter(|l| !l.trim().is_empty() && !l.starts_with("Listing") && !l.starts_with("WARNING"))
                    .map(|l| l.split_whitespace().next().unwrap_or("").split('/').next().unwrap_or("").to_string())
                    .filter(|s| !s.is_empty())
                    .collect()
            })
            .unwrap_or_default()
...
fn which(bin: &str) -> bool { std::process::Command::new("which").arg(bin).output().map(|o| o.status.success()).unwrap_or(false) }

fn has_internet() -> bool {
    Command::new("curl").args(["-s", "--max-time", "3", "https://www.google.com"])
...
fn search_menu(terminal: &mut Term, pm: Option<PackageManager>) -> Result<()> {
    ...
    loop {
        ...
        let mut choices: Vec<String> = results[start..end].iter().map(|r| {
            ...
            let status = if which(cmd) { "[installed]" } else { "[get]      " };
            format!("{status} {r}")
        }).collect();
        if page > 0 { choices.push("< Prev Page".into()); }
        if end < results.len() { choices.push("> Next Page".into()); }
        choices.push("---".into());
        choices.push("Back".into());
        let opts: Vec<&str> = choices.iter().map(String::as_str).collect();

        match run_menu(terminal, "Search Results", &opts, Some(&format!("Query: {query}  Page {}/{total_pages}", page+1)))? {
            MenuResult::Back => break,
            MenuResult::Selected(s) if s == "Back"        => break,
            MenuResult::Selected(s) if s == "> Next Page" => { page += 1; }
            MenuResult::Selected(s) if s == "< Prev Page" => { page = page.saturating_sub(1); }
...
fn install_pkg_dialog(terminal: &mut Term, pm: Option<PackageManager>, pkg: &str) -> Result<()> {
    ...
    if confirm(terminal, &format!("Install {pkg}?"))? {
        ...
        with_suspended(terminal, || { Command::new(&cmd[0]).args(&cmd[1..]).status()?; Ok(()) })?;
...
fn installed_menu(terminal: &mut Term, pm: Option<PackageManager>) -> Result<()> {
    ...
    loop {
        let filtered: Vec<&String> = installed.iter()
        ...
        let search_label = if filter.is_empty() { "Search...".to_string() } else { format!("Search: {filter}") };
        let mut choices = vec![search_label.clone(), "---".into()];
        choices.extend(filtered[start..end].iter().map(|p| format!("  {p}")));
        if page > 0      { choices.push("< Prev Page".into()); }
        if end < total   { choices.push("> Next Page".into()); }
        choices.push("---".into());
        choices.push("Back".into());
        let opts: Vec<&str> = choices.iter().map(String::as_str).collect();

        match run_menu(terminal, "Installed Apps", &opts, Some(&format!("{total} packages   Page {}/{total_pages}", page+1)))? {
            MenuResult::Back => break,
            MenuResult::Selected(s) if s == "Back"        => break,
            MenuResult::Selected(s) if s == "> Next Page" => { page += 1; }
            MenuResult::Selected(s) if s == "< Prev Page" => { page = page.saturating_sub(1); }
...
fn pkg_action_menu(terminal: &mut Term, pm: Option<PackageManager>, pkg: &str, installed: &mut Vec<String>) -> Result<()> {
    loop {
        match run_menu(terminal, pkg, &["Update", "Uninstall", "Add to Menu", "---", "Back"], None)? {
            MenuResult::Back => break,
            MenuResult::Selected(s) => match s.as_str() {
                "Update" => {
                    if let Some(pm) = pm {
                        if !has_internet() { flash_message(terminal, "No internet.", 800)?; continue; }
                ...
                "Add to Menu" => {
                    let menu_choice = run_menu(terminal, "Add to Menu", &["Applications", "Games", "Network", "---", "Back"], None)?;
                    if let MenuResult::Selected(m) = menu_choice {
                        if m != "Back" {
                            let display = input_prompt(terminal, &format!("Display name for '{pkg}':"))?
                                .unwrap_or_else(|| pkg.to_string());
                            let val = serde_json::Value::Array(vec![serde_json::Value::String(pkg.to_string())]);
                            match m.as_str() {
                                "Applications" => { let mut d = load_apps(); d.insert(display, val); save_apps(&d); }
                                "Games"        => { let mut d = load_games(); d.insert(display, val); save_games(&d); }
                                "Network"      => { let mut d = load_networks(); d.insert(display, val); save_networks(&d); }
                                _ => {}
                            }
