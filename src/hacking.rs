    ...
    for word in &words {
        ...
        for _ in 0..200 {
            ...
            if !idxs.iter().any(|i| used.contains(i)) {
                for (i, ch) in idxs.iter().zip(word.chars()) {
                    chars[*i] = ch;
                }
                used.extend(idxs);
...
fn find_word_at(idx: usize, positions: &[WordPos]) -> Option<&WordPos> {
    positions
        .iter()
        .find(|wp| idx >= wp.start && idx < wp.start + WORD_LEN)
}
...
fn draw_grid_frame(
    ...
    terminal.draw(|f| {
        ...
        // Hint
        f.render_widget(
            Paragraph::new("TAB/q=cancel").style(ds),
            Rect {
                x: H_PAD,
        ...
        for col_block in 0..COLS {
            for row in 0..ROWS {
                ...
                if sy >= size.height.saturating_sub(2) {
                    continue;
                }
                f.render_widget(
        ...
        for (i, &ch) in grid.chars.iter().enumerate() {
            ...
            if sy >= size.height.saturating_sub(2) || sx >= size.width {
                continue;
            }
            ...
            let highlighted = hover_word.is_some_and(|hw| i >= hw.start && i < hw.start + WORD_LEN)
