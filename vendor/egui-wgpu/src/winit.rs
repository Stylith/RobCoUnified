...
use crate::{
    current_crt_effects, renderer, CrtFrameState, CrtPipeline, RenderState, SurfaceErrorAction,
    WgpuConfiguration,
};
...
pub struct Painter {
    ...
    crt_pipeline: Option<CrtPipeline>,

    instance: wgpu::Instance,
    render_state: Option<RenderState>,

    // Per viewport/window:
    crt_frame_state: ViewportIdMap<CrtFrameState>,
...
impl Painter {
    ...
    pub fn new(
        ...
        Self {
            ...
            crt_frame_state: Default::default(),
    ...
    pub fn paint_and_update_textures(
        ...
        let final_frame_view = if capture {
            Self::update_capture_state(
                &mut self.screen_capture_state,
                &output_frame,
                &render_state,
            );
            self.screen_capture_state.as_ref().map_or_else(
                || {
                    output_frame
                        .texture
                        .create_view(&wgpu::TextureViewDescriptor::default())
        ...
        let crt_effects = current_crt_effects();
        if crt_effects.is_none() {
            self.crt_frame_state.clear();
            self.crt_pipeline = None;
        }
        if let Some(effects) = crt_effects {
            if self.crt_pipeline.is_none() {
                self.crt_pipeline = Some(CrtPipeline::new(
                    &render_state.device,
                    render_state.target_format,
                ));
            }
            if self.crt_frame_state.get(&viewport_id).is_none_or(|state| {
                state.width != surface_state.width || state.height != surface_state.height
            }) {
                let frame_state = self
                    .crt_pipeline
                    .as_ref()
                    .expect("CRT pipeline missing")
                    .create_frame_state(
                        &render_state.device,
                        render_state.target_format,
                        surface_state.width,
                ...
                for history_view in frame_state.history_views() {
                    let _clear_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                        label: Some("egui_crt_history_clear"),
                ...
                self.crt_frame_state.insert(viewport_id, frame_state);
            }
            if let Some(crt_pipeline) = self.crt_pipeline.as_ref() {
                crt_pipeline.update_uniforms(
                    &render_state.queue,
                    effects,
                    surface_state.width,
                    surface_state.height,
...

        {
            ...
            let output_view = crt_effects
                .and_then(|_| {
                    self.crt_frame_state
                        .get(&viewport_id)
                        .map(|state| &state.view)
                })
                .unwrap_or(&final_frame_view);
            let (view, resolve_target) = (self.msaa_samples > 1)
                .then_some(self.msaa_texture_view.get(&viewport_id))
                .flatten()
                .map_or((output_view, None), |texture_view| {
                    (texture_view, Some(output_view))
                });
        ...
        if let (Some(effects), Some(crt_pipeline), Some(state)) = (
            crt_effects,
            self.crt_pipeline.as_ref(),
            self.crt_frame_state.get(&viewport_id),
        ) {
            if effects.bloom > 0.0 {
                let bloom_extract_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                    label: Some("egui_crt_bloom_extract"),
                    color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                        view: state.bloom_ping_view(),
                        resolve_target: None,
                        ops: wgpu::Operations {
                            load: wgpu::LoadOp::Clear(wgpu::Color::BLACK),
                            store: wgpu::StoreOp::Store,
                ...
                crt_pipeline.paint_bloom_extract(
                    &mut bloom_extract_pass.forget_lifetime(),
                    state.bloom_extract_bind_group(),
                );

                let bloom_blur_h_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                    label: Some("egui_crt_bloom_blur_h"),
                    color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                        view: state.bloom_pong_view(),
                        resolve_target: None,
                        ops: wgpu::Operations {
                            load: wgpu::LoadOp::Clear(wgpu::Color::BLACK),
                            store: wgpu::StoreOp::Store,
                ...
                crt_pipeline.paint_bloom_blur_h(
                    &mut bloom_blur_h_pass.forget_lifetime(),
                    state.bloom_ping_bind_group(),
                );

                let bloom_blur_v_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                    label: Some("egui_crt_bloom_blur_v"),
                    color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                        view: state.bloom_ping_view(),
                        resolve_target: None,
                        ops: wgpu::Operations {
                ...
                crt_pipeline.paint_bloom_blur_v(
                    &mut bloom_blur_v_pass.forget_lifetime(),
                    state.bloom_pong_bind_group(),
                );
            } else {
                for bloom_view in state.bloom_views() {
                    let _clear_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                        label: Some("egui_crt_bloom_clear"),
                        color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                            view: bloom_view,
                            resolve_target: None,
                            ops: wgpu::Operations {
        ...
        if let Some(crt_pipeline) = self.crt_pipeline.as_ref() {
            if let Some(state) = self.crt_frame_state.get(&viewport_id) {
                let render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                    label: Some("egui_crt_render"),
                    color_attachments: &[
                        Some(wgpu::RenderPassColorAttachment {
                            view: &final_frame_view,
                            resolve_target: None,
                            ops: wgpu::Operations {
                                load: wgpu::LoadOp::Clear(wgpu::Color::BLACK),
                                store: wgpu::StoreOp::Store,
                        ...
                        Some(wgpu::RenderPassColorAttachment {
                            view: state.history_target_view(),
                            resolve_target: None,
                            ops: wgpu::Operations {
                                load: wgpu::LoadOp::Clear(wgpu::Color::BLACK),
                                store: wgpu::StoreOp::Store,
                ...
                crt_pipeline.paint(&mut render_pass.forget_lifetime(), state.bind_group());
            }
            if let Some(state) = self.crt_frame_state.get_mut(&viewport_id) {
