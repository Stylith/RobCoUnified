...
fn crt_pointer_curve_lock() -> &'static RwLock<Option<f32>> {
    static CRT_POINTER_CURVE: OnceLock<RwLock<Option<f32>>> = OnceLock::new();
...
fn apply_crt_pointer_curve(
    ...
    if curvature <= 0.0 || screen_size_in_points.x <= 0.0 || screen_size_in_points.y <= 0.0 {
        return Some(pos_in_points);
    }

    let uv = egui::vec2(
        pos_in_points.x / screen_size_in_points.x,
        pos_in_points.y / screen_size_in_points.y,
    );
    let centered = uv * 2.0 - Vec2::splat(1.0);
    let r2 = centered.x * centered.x + centered.y * centered.y;
    let warped = centered * (1.0 + curvature * r2);
    let curved_uv = warped * 0.5 + Vec2::splat(0.5);

    if !(0.0..=1.0).contains(&curved_uv.x) || !(0.0..=1.0).contains(&curved_uv.y) {
        return None;
    }

    Some(egui::pos2(
        curved_uv.x * screen_size_in_points.x,
        curved_uv.y * screen_size_in_points.y,
    ))
}
...
impl State {
    ...
    fn on_cursor_moved(
        ...
        let screen_size_in_points = screen_size_in_pixels(window) / pixels_per_point;
        let mapped_pos_in_points = if let Some(curvature) = current_crt_pointer_curve() {
            apply_crt_pointer_curve(raw_pos_in_points, screen_size_in_points, curvature)
        } else {
            Some(raw_pos_in_points)
        };
        let Some(pos_in_points) = mapped_pos_in_points else {
            self.pointer_pos_in_points = None;
            self.egui_input.events.push(egui::Event::PointerGone);
            return;
        };
