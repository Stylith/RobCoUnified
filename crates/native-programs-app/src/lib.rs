...
pub fn build_terminal_application_entries(
    ...
    entries.extend(
        configured_names
            .iter()
            .filter(|name| *name != text_editor_label && *name != BUILTIN_FILE_MANAGER_APP)
            .cloned(),
    );
    entries
}
...
pub fn resolve_terminal_applications_request(
    ...
    match resolve_terminal_applications_action(event, text_editor_label) {
...
mod tests {
    ...
    fn resolve_desktop_applications_action_maps_builtin_labels() {
        assert_eq!(
            resolve_desktop_applications_action("Editor", "Editor"),
            DesktopApplicationsAction::OpenTextEditor
        );
        assert_eq!(
            resolve_desktop_applications_action("Custom", "Editor"),
